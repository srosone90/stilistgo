...
export default function AuthForm() {
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      if (err) { setError(err.message); return; }
      // Notifica via email (fire-and-forget)
  ...
  const modeTitle: Record<Mode, string> = {
    login: 'Accedi al gestionale',
    register: 'Crea il tuo account',
    forgot: 'Reimposta password',
    reset: 'Scegli nuova password',
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center mb-8">
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--muted)' }}>{modeTitle[mode]}</p>
      </div>
          ...
          {showResend && (
            <button
              ...
              style={{
                ...
                borderRadius: '6px', color: '#fca5a5', padding: '5px 12px', fontSize: '12px',
                cursor: resendLoading ? 'not-allowed' : 'pointer', opacity: resendLoading ? 0.6 : 1,
              }}
            >
      ...
      {success && (
        <div
          className="mb-4 px-4 py-3 rounded-xl text-sm"
          style={{ background: 'rgba(34,197,94,0.1)', borderLeft: '3px solid #22c55e', color: '#86efac' }}
        >
          {success}
        </div>
      )}
      ...
      {mode === 'reset' && (
        <div className="space-y-4">
          ...
          >
            {loading ? <><Spinner />Salvataggio...</> : 'Salva nuova password'}
          </button>
        </div>
      )}
